...
class ApiControllerIntegrationTest {
    ...
    @Test
    @Order(13)
    @DisplayName("GET /api/events — returns a JSON array (activity log feed)")
    void eventsReturnsArray() throws Exception {
        mockMvc.perform(get("/api/events"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    }

    // ── 14. Audit exposes dashboard counters ─────────────────────────────────

    @Test
    @Order(14)
    @DisplayName("GET /api/audit — summary includes duplicatesDropped and volumeSettled")
    void auditIncludesDashboardCounters() throws Exception {
        mockMvc.perform(get("/api/audit"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.summary.duplicatesDropped").exists())
                .andExpect(jsonPath("$.summary.volumeSettled").exists());
    }

    // ── 15. Device connectivity toggle ───────────────────────────────────────

    @Test
    @Order(15)
    @DisplayName("POST /api/mesh/devices/{id}/internet — toggles a device; unknown device is 400")
    void deviceInternetToggle() throws Exception {
        mockMvc.perform(post("/api/mesh/devices/phone-bridge/internet")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"enabled\": true}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.deviceId").value("phone-bridge"))
                .andExpect(jsonPath("$.hasInternet").value(true));

        mockMvc.perform(post("/api/mesh/devices/no-such-phone/internet")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"enabled\": true}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").exists());
    ...
    @Test
    @Order(16)
    @DisplayName("POST /api/demo/tamper — a tampered packet is INVALID")
    void tamperDemoIsRejected() throws Exception {
        String body = """
            { "senderVpa": "alice@demo", "receiverVpa": "bob@demo", "amount": 1.00, "pin": "1234" }
            """;
        mockMvc.perform(post("/api/demo/tamper")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome").value("INVALID"))
                .andExpect(jsonPath("$.reason").value("decryption_failed"));
    }

    // ── 17. Concurrent bridges demo ──────────────────────────────────────────

    @Test
    @Order(17)
    @DisplayName("POST /api/demo/concurrent-upload — 3 bridges: exactly 1 settles, 2 dropped")
    void concurrentUploadSettlesOnce() throws Exception {
        String body = """
            { "senderVpa": "alice@demo", "receiverVpa": "bob@demo", "amount": 1.00, "pin": "1234", "bridges": 3 }
            """;
        mockMvc.perform(post("/api/demo/concurrent-upload")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.bridges").value(3))
                .andExpect(jsonPath("$.settled").value(1))
                .andExpect(jsonPath("$.duplicatesDropped").value(2));
    }
    ...
    @Test
    @Order(18)
    @DisplayName("POST /api/demo/send — 400 when sender and receiver are the same account")
    void demoSendRejectsSelfPayment() throws Exception {
        String body = """
            { "senderVpa": "alice@demo", "receiverVpa": "alice@demo", "amount": 10, "pin": "1234" }
            """;
        mockMvc.perform(post("/api/demo/send")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
