...
public class MeshSimulatorService {
    ...
    public int hopCount(MeshPacket packet) {
        ...
        return start == null ? 0 : Math.max(0, start - packet.getTtl());
    }

    /** Give a device connectivity (it becomes a bridge) or take it away (it goes back to BLE-only). */
    public VirtualDevice setInternet(String deviceId, boolean enabled) {
        VirtualDevice d = devices.get(deviceId);
        if (d == null) throw new IllegalArgumentException("Unknown device: " + deviceId);
        d.setInternet(enabled);
        log.info("Device {} connectivity set to {}", deviceId, enabled ? "4G" : "offline");
        return d;
    }
